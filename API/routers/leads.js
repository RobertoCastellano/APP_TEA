const express = require('express');
const router = express.Router();
const db = require('../config/db');
const { read } = require('fs');

// VARIABLES POR URL --> req.params
// VARIABLES POR BODY --> req.body

	router.post('/add', async (req, res) => {

		const required = ["ID_user", "name", "phone", "email_lead"];
		
		// Comprobamos que estén todos los campos obligatorios
		const security = required.find((e)=>{
			if( req.body[e] === undefined ){ return true; }
		})

		if( security !== undefined){
			return res.json(`Falta el campo ${security}`);
		}
		const obj = {
			ID_user: 	req.body.ID_user,
			name: 		req.body.name,
			phone: 		req.body.phone,
			email_lead:	req.body.email_lead
		};
		const result = await db('leads').insert(obj);
		
		res.json(`Se acaba de añadir a ${result[0]}`);
		//res.json(`Se acaba de añadir a ${req.body.name}`);
	});

	router.put('/edit/:id',async (req, res) => {

		const result = await db('leads')
		.update({
			name: 		req.body.name,
			phone: 		req.body.phone,
			email_lead:	req.body.email_lead
		})
		.where("ID", req.params.id);

		res.json({ status: true });
		res.json("EDITAMOS UN LEAD "+req.params.id);
	});

	router.delete('/delete/:id', async (req, res) => {
		
		await db('leads')
		.where('ID', req.params.id)
		.delete();

		res.json({status:true});
		res.json("BORRAMOS UN LEAD "+req.params.id);
	});

	router.get('/select/:id', async (req, res) => {

		const resultado = await db
		.select("*")
		.from('leads')
		.where("ID", req.params.id);

		if(resultado.length === 0){
			res.status(400).json({
				status: false,
				error: "No existe el ID indicado"
			})
		}else{
			res.json({
				status: true,
				data: resultado[0]
			});
		}
		res.json("SELECCIONAMOS UN LEAD "+req.params.id);
	});

	router.get('/list', async (req, res) => {

		const resultado = await db
		.select("*")
		.from('leads');

		res.json({
			status: true,
			data: resultado
		});
		res.json("SELECCIONAMOS VARIOS LEADS "+req.params.id);
	});

module.exports = router;